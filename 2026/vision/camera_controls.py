...
def set_camera_robust_defaults(camera, frc_config, camera_type="c920", delay=0):
    ...
    if exp_prop:
        ...
        # B. Force v4l2-ctl on Linux
        if sys.platform.startswith("linux") and (camera_type == "c920" or camera_type == "arducam"):
            path = camera.getPath()
            # Allow /dev/videoX AND /dev/v4l/by-id/... paths
            if path.startswith("/dev/"):
                # Arducam uses 'auto_exposure', C920 uses 'exposure_auto'
                ctrl_name = "auto_exposure" if camera_type == "arducam" else "exposure_auto"
                try:
                    # Force Manual Mode (1)
                    subprocess.run(["v4l2-ctl", "-d", path, f"--set-ctrl={ctrl_name}=1"], check=False, capture_output=True)
